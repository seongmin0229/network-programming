      ...
      while (keys.hasNext()) {
        ...
        try {
          if (key.isAcceptable()) {
            ...
            //System.out.println("writable channel: " + channel);
...
//    }
    ...
    try {
      ...
      String contentType = 
          URLConnection.getFileNameMap().getContentTypeFor(f);
      Path file = FileSystems.getDefault().getPath(f);
      byte[] data = Files.readAllBytes(file);
      ByteBuffer input = ByteBuffer.wrap(data);
      
      /*/sol1: for korean text (doesn't work)
      Charset charset=Charset.forName("EUC-KR");
      CharsetEncoder encoder = charset.newEncoder();
      String strData=new String(data);
      ByteBuffer input = encoder.encode(CharBuffer.wrap(strData));
      //*/
      
//      ByteBuffer input = ByteBuffer.wrap(data);
      
      /*/sol2: for korean text
